...
В соответствии с DDD были выделены следующие домены, поддомены и контексты:
- Домен: Удаленное управление устройствами
  - Поддомен: Управление отоплением в доме
    - Контекст: Включение/отключение отопления
    - Контекст: Задание требуемой температуры отопления
    - Контекст: Получение текущей температуры отопления

> Данный список отражает область автоматизации текущей системы Smart Home, т.к. задание относится описанию текущей системы.
...
- Упрощает развертывание, управление и обслуживание системы, т.к. она представлена одним приложением.
- Простота в отладке.
- Консистентность данных: транзакции не выходит за границы экземпляра приложения.
